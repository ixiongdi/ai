# ID algorithm decision tree

Walk **top to bottom**. Stop at the first matching row unless the user
overrides. Algorithms follow
[ixiongdi/id-generator](https://github.com/ixiongdi/id-generator)
recommendations, independent of language.

Generate with `python scripts/idgen.py generate <slug>`.
...
`uuid_v8`, `atomic_id`. The Python script only simulates these locally.
...
`scripts/idgen.py` (and the TypeScript MCP) show format. They do not
assign cluster worker IDs or a Redis counter for you.
